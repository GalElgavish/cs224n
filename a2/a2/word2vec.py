...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow.
    y_hat = softmax(outsideVectors @ centerWordVec)  # (|V|,)
    y = np.zeros_like(y_hat)  # (|V|,)
    y[outsideWordIdx] = 1
...
def negSamplingLossAndGradient(
    ...
    U_o_ws = np.column_stack((outsideVectors[outsideWordIdx], -outsideVectors[negSampleWordIndices].T))  # (d, K+1)
    sig_U_v = sigmoid(U_o_ws.T @ centerWordVec)  # (K+1,)
    loss = -np.sum(np.log(sig_U_v))  # scalar
    gradCenterVec = U_o_ws @ (sig_U_v - 1)  # (d,)
    gradOutsideVecs = np.zeros(outsideVectors.shape)  # (|V|, d)
    # dJ_du_o
    gradOutsideVecs[outsideWordIdx] = (sig_U_v[0] - 1)*centerWordVec  # (d,)
    # dJ_d-w_s
    for k, negSampleWordIdx in enumerate(negSampleWordIndices):
        gradOutsideVecs[negSampleWordIdx] -= (sig_U_v[k+1] - 1)*centerWordVec  # (d,)
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        loss += loss_j
        gradCenterVecs[word2Ind[currentCenterWord]] += gradCenterVec_j
